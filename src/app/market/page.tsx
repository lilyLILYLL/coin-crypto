...
const Market = () => {
    ...
    React.useEffect(() => {
        axios
            ...
            .then((res) => {
                const map = res.data.map((coin) => ({ src: coin.image, name: coin.name, price: coin.current_price, atl: coin.atl, rank: coin.rank, market_cap: coin.market_cap } as CoinModel));
                setCoins(map);
            })
            ...
            <table className="lg:w-full mx-auto rounded-xl w-[800px]">
                <thead className="">
                    <tr className="bg-gradient-to-r  from-blue to-pink ">
                        ...
                        <th scope="col" className={`${defaultTextStyle} w-[30%] text-xl px-6 py-6 text-right`}>
                            Market Cap
                        </th>
                    </tr>
                </thead>
                ...
                <tbody>
                    {coins?.map((coin, index) => (
                        <tr key={index} className="border-b border-gray hover:bg-darkPurple ">
                            ...
                            <td className={`${defaultTextStyle} text-xl px-6 py-6 text-right`}> $ {numberWithCommas(coin.market_cap || 0)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
